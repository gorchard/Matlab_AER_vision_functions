    ...
    methods(Static)
        function anno = read_cpp_file(filename)
            if exist(filename, 'file')
                ...
                num_points = floor(file_size/((52+12)*numtracks+8)); %52 for the skip, 12 for each tracker data, 8 for the time
                ...
                for ii = 1:numtracks
                    trk(ii).x = zeros(1,num_points);
                    trk(ii).y =zeros(1,num_points);
                    trk(ii).width =zeros(1,num_points);
                    trk(ii).height = zeros(1,num_points);
                    trk(ii).ID = zeros(1,num_points);
                    trk(ii).active = zeros(1,num_points);
                    trk(ii).ts = zeros(1,num_points);
                ...
                for point_num = 1:num_points
                    ...
                    for ii = 1:numtracks
                        ...
                        if(active)
                            ...
                            point = annotation_point(x-width, y-height, width*2, height*2, time);
                            new_track(ID).add_point(point);
...
%                 fgets(fileID); %for the newline character etc
    ...
    methods(Access = public)
        ...
        function bounding_boxes = get_all_bounding_boxes(annotation, time_point)
